...
First, this method should call on the `help` method to show the user the available commands. Then, it should `puts` out the prompt: `q`. It should capture the user's response using `gets.chomp` or `gets.strip`.
